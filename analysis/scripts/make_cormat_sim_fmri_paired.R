...
Start.time <- Sys.time()
my.corrmats <- make_paired_corrmats(user.adjacency=NULL,      
...
my.corrmats$baseline.mats[[1]]$corrmat
my.corrmats$followup.mats[[1]]

lab.mat <- matrix(0, nrow=nrow(my.corrmats$baseline.mats[[1]]$corrmat), ncol=ncol(my.corrmats$baseline.mats[[1]]$corrmat))
for(i in 1:length(my.corrmats$baseline.mats[[1]]$sig.vars)){
  
  n.pairs <- length(my.corrmats$baseline.mats[[1]]$interacting.partners[[i]])
  if(n.pairs > 0){
    
    for(j in 1:n.pairs){
      
      lab.mat[my.corrmats$baseline.mats[[1]]$sig.vars[i], my.corrmats$baseline.mats[[1]]$interacting.partners[[i]][j]] <- 1
      lab.mat[my.corrmats$baseline.mats[[1]]$interacting.partners[[i]][j], my.corrmats$baseline.mats[[1]]$sig.vars[i]] <- 1
...
p1 <- ggcorrplot(my.corrmats$baseline.mats[[1]]$corrmat, hc.order = FALSE, type = "lower", colors=c("darkblue","white","darkred"),
                 lab = F, show.diag=T, title="Baseline Correlation Matrix", p.mat=lab.mat, sig.level=.05, insig="pch", pch=8, pch.cex=0.9) +
  theme(plot.title=element_text(size=14, hjust=0.5))
p2 <- ggcorrplot(my.corrmats$followup.mats[[1]], hc.order = FALSE, type = "lower", colors=c("darkblue","white","darkred"),
                 lab = F, show.diag=T, title="Follow-up Correlation Matrix", p.mat=lab.mat, sig.level=.05, insig="pch", pch=8, pch.cex=0.9) +
  theme(plot.title=element_text(size=14, hjust=0.5))

ggarrange(p1, p2, nrow=1, ncol=2, common.legend=T, align="hv")
ggsave("2by2_paired_corrplots_functional-labeled.pdf", height=7, width=12)
