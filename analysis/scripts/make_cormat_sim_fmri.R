...
### make example case and control correlation matrices with differential correlation
...
g <- igraph::erdos.renyi.game(num.variables, prob) # erdos-renyi network - can also be created from real data by thresholding real correlation matrices
...
start.time <- Sys.time()
my.corrmats <- make_case_ctrl_corrs(user.adjacency=NULL,      
                     graph.type="Erdos-Renyi", 
                     hi.cor=0.9, lo.cor=-0.9,   
                     multiway=10,              
                     num.variables=100,         
                     prob.connected=0.5,      
                     out.degree=NULL,          
...
my.corrmats$cases[[1]]$corrmat
my.corrmats$ctrls[[1]]$corrmat
# make summary plots
make_sim_plot(my.corrmats$cases[[1]], my.corrmats$ctrls[[1]], n1=30, n2=30, save.plot=F)

my.corrmats$cases[[1]]$sig.vars
my.corrmats$cases[[1]]$interacting.partners

lab.mat <- matrix(0, nrow=nrow(my.corrmats$cases[[1]]$corrmat), ncol=ncol(my.corrmats$cases[[1]]$corrmat))
for(i in 1:length(my.corrmats$cases[[1]]$sig.vars)){
  
  n.pairs <- length(my.corrmats$cases[[1]]$interacting.partners[[i]])
  if(n.pairs > 0){
    
    for(j in 1:n.pairs){
      
      lab.mat[my.corrmats$cases[[1]]$sig.vars[i], my.corrmats$cases[[1]]$interacting.partners[[i]][j]] <- 1
      lab.mat[my.corrmats$cases[[1]]$interacting.partners[[i]][j], my.corrmats$cases[[1]]$sig.vars[i]] <- 1
...
p1 <- ggcorrplot(my.corrmats$cases[[1]]$corrmat, hc.order = FALSE, type = "lower", colors=c("darkblue","white","darkred"),
                 lab = F, show.diag=T, title="Case Correlation Matrix", p.mat=lab.mat, sig.level=.05, insig="pch", pch=8, pch.cex=0.9) +
                 theme(plot.title=element_text(size=14, hjust=0.5))
p2 <- ggcorrplot(my.corrmats$ctrls[[1]]$corrmat, hc.order = FALSE, type = "lower", colors=c("darkblue","white","darkred"),
                 lab = F, show.diag=T, title="Control Correlation Matrix", p.mat=lab.mat, sig.level=.05, insig="pch", pch=8, pch.cex=0.9) +
                 theme(plot.title=element_text(size=14, hjust=0.5))

ggarrange(p1, p2, nrow=1, ncol=2, common.legend=T, align="hv")
ggsave("2by2_case-control_corrplots_functional-labeled.pdf", height=7, width=12)

case.mat <- my.corrmats$cases[[1]]$corrmat
ctrl.mat <- my.corrmats$ctrls[[1]]$corrmat
diag(case.mat) <- NA
diag(ctrl.mat) <- NA
meh <- melt(case.mat, na.rm=T)
meh2 <- melt(ctrl.mat, na.rm=T)
head(cbind(meh, meh2), n=20)
